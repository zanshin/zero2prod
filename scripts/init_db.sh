...
# Launch postgres using Docker unless it is already running
if [[ -z "${SKIP_DOCKER}" ]]
then
  docker run \
    ...
    -d postgres \
    postgres -N 1000
    # ^ Increased maximum number of connections for test purposes
fi
...
>&2 echo "Posstgres has been migrated, ready to go"
